...
(Update: After finishing this post, while doing the dishes, I realized that the
"primordial soup" approach might actually work, after all. If you've got hundreds
of examples where the completion is very similar to the prompt, the LLM might learn
through the fine-tuning process that it's role is to just edit the prompt.)
